...
func (h *ProxyHandler) Handle(c *gin.Context) {
	...
	// Proxy the request
	h.proxyRequest(c, rawPath)
}

func (h *ProxyHandler) proxyRequest(c *gin.Context, targetURL string) {
	h.doProxy(c, targetURL, false, 0)
}

func (h *ProxyHandler) doProxy(c *gin.Context, targetURL string, followRedirects bool, depth int) {
	if depth > 10 {
	...
	if location := resp.Header.Get("Location"); location != "" {
		if checkURL(location) {
			...
			h.doProxy(c, location, true, depth+1)
			return
		}
	}
